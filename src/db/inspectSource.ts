...
async function modeSnapshot(client: Client, args: Record<string, string | boolean>) {
  ...
  const ev = await client.query(
    `SELECT id, slug, title, description, artists, venue_id, sub_venue, start_datetime, primary_genre, genre_confidence, confidence,
            published, cancelled, manual_override, overridden_fields, admin_unpublish_reason,
            source_cancelled_at, source_cancelled_by_source_id, source_cancellation_evidence,
            official_event_url, ticket_url, resident_advisor_url, facebook_url, image_url, price_from, created_at
