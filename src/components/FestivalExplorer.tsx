    ...
    <div>
      <div className="flex flex-wrap items-center gap-2">
        {/* text-base (16px) on mobile, not text-xs (12px): iOS Safari
            ...
            vertical padding drops one more step (py-1) and the border
            switches to the dimmer --border token on mobile only, each
            restored to its original desktop value at sm+ (sm:py-1.5,
            sm:border-border-strong) — same base+sm: pattern throughout.
            New mobile height: 4px+4px padding + 16px line-height (leading-4)
            + 1px+1px border = 26px, above the WCAG 2.2 AA 24px minimum
            target size for a compact secondary filter control (still well
            short of any accidental shrink toward the 16px iOS-zoom
            threshold, which only concerns font-size, left untouched). */}
        <select
          value={country}
          onChange={(e) => setCountry(e.target.value)}
          className="rounded-full border border-border bg-surface-1 px-2.5 py-1 sm:border-border-strong sm:px-3 sm:py-1.5 text-base leading-4 sm:text-xs font-medium sm:font-semibold uppercase tracking-normal sm:tracking-wide text-text-secondary hover:text-text-primary"
        >
