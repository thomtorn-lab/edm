...
describe("decidePublishedEventSyncAction (data-quality Workstream A follow-up — existing published events that now resolve to HOLD)", () => {
  ...
  it("admin-unpublished + AUTO -> stays unpublished (admin unpublish/cancellation safety, 2026-09-06) — a fresh 'auto_publish' quality-gate verdict for an admin-unpublished event (e.g. the source later stops saying 'cancelled') must never flip it back live on its own; !current.published short-circuits before manualOverride/holdReason are even consulted, exactly as for any other already-unpublished event", () => {
    expect(
