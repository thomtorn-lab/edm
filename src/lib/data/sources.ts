...
export const SOURCES: Source[] = [
  ...
  // ---- Billetto: ticketing discovery, verification-strength for date/venue/status ----
  {
    ...
    integrationNote:
      "Phase 1 diagnosis (2026-08-20, Electronic CPH ingestion task), two passes. Pass 1 confirmed via a live GitHub Actions run that neither BILLETTO_ACCESS_KEY_ID nor BILLETTO_ACCESS_KEY_SECRET existed as a repository secret at all (job failed at the credential-presence check, before any request). Pass 2, after both secrets were added: same live-run approach, GET https://billetto.dk/api/v3/public/events with the documented Api-Keypair header now returns HTTP 401 {\"error\":{\"message\":\"Invalid credentials\",\"type\":\"authentication_error\"}} — the secrets exist but the credential PAIR itself is rejected. Also confirmed billetto.dk (not api.billetto.com) is the real API host: api.billetto.com/v3/public/events returns 404 and is actually Billetto's ReadMe.io-hosted API *documentation* portal (readme-subdomain: billetto, docs HTML body), not a live API server — so this is not a wrong-host problem, it's a genuinely invalid key pair. Still not a rejection of Billetto as a concept — the endpoint, header scheme and pagination (`after` cursor, max limit 100) are all documented and Billetto's own guidance permits this discovery/link-back use case. Next step: verify the BILLETTO_ACCESS_KEY_ID/BILLETTO_ACCESS_KEY_SECRET values themselves against Billetto's publisher/developer dashboard (active key pair, no stray whitespace/newline from copy-paste into the GitHub secret) — this needs the user's own Billetto account access, not something to self-provision. Once a valid pair is confirmed, re-run the same live-run diagnosis before writing an adapter: real field schema, the best available Copenhagen filter (no location filter has been verified yet — plan is q=/city=/location= candidate probing against a live sample), and real dedup outcomes against the two known overlap events (Hangaren's \"Arcanum Collective: POSSESSED\", Poolen's \"Infected Mushroom\") through the now-merged evidence-based dedup model. Do not scrape the unauthenticated HTML listing page as a substitute — the documented API is the permitted path here.",
  },
