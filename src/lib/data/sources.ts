...
export const SOURCES: Source[] = [
  ...
  // ---- Billetto: ticketing discovery, verification-strength for date/venue/status ----
  {
    ...
    integrationNote:
      "Phase 1 diagnosis (2026-08-20, Electronic CPH ingestion task): confirmed via a real, live GitHub Actions run against the documented public endpoint (GET https://billetto.dk/api/v3/public/events, Api-Keypair header) that neither BILLETTO_ACCESS_KEY_ID nor BILLETTO_ACCESS_KEY_SECRET exists as a repository secret — the job failed at the credential-presence check before any request was made (0 events sampled, no schema/overlap data obtained). This supersedes the 2026-08-18 pass's untested assumption of a credential gate: it is now a confirmed hold, not a hypothesis. Still not a rejection — the endpoint, header scheme and pagination (`after` cursor, max limit 100) are all documented and Billetto's own guidance permits this discovery/link-back use case. Once both secrets are added (Settings > Secrets and variables > Actions > Secrets), re-run the same diagnosis before writing an adapter: confirm the real field schema, the best available Copenhagen filter (no location filter has been verified — plan is q=/city=/location= candidate probing against a live sample), and real dedup outcomes against the two known overlap events (Hangaren's \"Arcanum Collective: POSSESSED\", Poolen's \"Infected Mushroom\") through the now-merged evidence-based dedup model. Do not scrape the unauthenticated HTML listing page as a substitute — the documented API is the permitted path here.",
  },
