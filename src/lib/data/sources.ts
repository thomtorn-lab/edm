...
export const SOURCES: Source[] = [
...

  {
    ...
    eventsUpdated: 0,
    integrationNote:
      "Implemented 2026-08-30 (source-expansion work package). Real, unmodified fixtures (three full event blocks captured live via inspect-source.yml's reachability mode) — see rustAdapter.test.ts. A real production parsing gotcha was found and fixed live: every event article's own opening tag embeds an Alpine.js arrow function (`$nextTick( () => ... )`) whose own `=>` contains a literal `>`, which breaks a naive `[^>]*`-bounded tag match — the adapter instead splits on `<article`/`</article>` boundaries and filters by the itemtype marker, confirmed against the real page. No dedicated `/event/{slug}` URL is exposed from the homepage listing itself, so officialEventUrl uses a stable `#event-<wordpress-post-id>` anchor derived from the page's own real per-event ID (used for its own in-page toggle), not invented. Wired into the sync API route (src/app/api/sync/[source]/route.ts) but deliberately NOT given a recurring GitHub Actions sync-rust.yml workflow yet, matching the same 'onboard first, schedule later' precedent already used for ALICE.\n\n" +
      "FINAL ONBOARDING REVIEW (2026-08-30, before any merge): a full walk of the live homepage (110 real events, 100% parsed) found the venue's own event permalinks discovered by earlier web research (Sol:Luna, MANA CLUB, DUST, Sol:luna Halloween Special) all carry a 2025 `startDate` — already past, not current 2026 coverage; retracted as coverage-gain evidence. The ONE verified current (2026) event that resolves 'strong' under the existing relevance code — 'Electronic Equinox Gathering: CRYO + NEONSOL + NEOTEK' (Sep 25) — is a mixed live-band-plus-DJ-afterparty bill spanning synth/goth/industrial/EBM/postpunk/darkwave; a careful first-party read makes this a manual-review case, not a clean auto-publish, because 'goth' and 'postpunk' (rock/band genres) are not tracked as non-electronic evidence at all in relevance.ts's NON_ELECTRONIC_GENRE_SIGNALS — 'goth' is absent from the list entirely, and 'postpunk' (written as one unhyphenated word, same compounding pattern already documented for 'byvandring'/'øl') evades the existing `\\bpunk\\b` word-boundary regex. This was a general relevance-classification gap, not something specific to RUST — any source whose first-party text uses 'goth' or a compounded 'postpunk' was equally exposed. FIXED 2026-08-30 in relevance.ts (goth/gothic and postpunk/post-punk/post punk added to NON_ELECTRONIC_GENRE_SIGNALS, reusing the existing graduated assessRelevance logic — never hardcoded to this one event or source; industrial/EBM/darkwave on their own remain real EDM evidence and still resolve 'strong'). Electronic Equinox Gathering now correctly resolves 'weak' (manual review), confirmed against the live code. Current verified value: 0 clean auto-publish, 1 manual-review candidate, 0 confirmed live-venue gain until that candidate is resolved one way or the other.",
  },
